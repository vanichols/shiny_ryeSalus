...
ccbio_dist <-
  ...
  mutate(
    ...
    ccbio_lbs = round(ccbio_kgha * 2.2 / 2.47, 0),
    ccbio_lbs = ifelse(ccbio_lbs < 0, 0, ccbio_lbs),
    dop_nice = factor(dop, labels = c(
      "Planted Sep-15", "Planted Oct-7", "Planted Nov-1"
    )),
    dot_nice = factor(
      dot,
      labels = c("Apr-1", "Apr-15", "May-1", "May-15", "Jun-1", "Jun-15")
    ),
    dot_nicerev = fct_rev(dot_nice)
  )
...
#--dropdowns
dd_county_dist <-
  ccbio_dist %>% select(subregion) %>% distinct() %>% pull()
...
#--dropdowns
dd_county_sum <-
  ccbio_sum %>% select(subregion) %>% distinct() %>% pull()
...
ui <- fluidPage(
  ...
  # Application title
  navbarPage(
    "Iowa Cover Crop Biomass Explorer",
    ...
    tabPanel(
      ...
      fluidRow(
        column(6,
               plotOutput('fig_sum', height = 400, width = 450)
               ),
        column(6,
        #h3("Expected Cover Crop Biomass at Termination (lbs/ac)"),
    ...
    tabPanel(
      ...
      fluidRow(
        ...
        column(4,
               plotOutput(
                 'fig_map_dist', width = 400, height = 350
               )),
        column(4,
               fluidRow(
                 column(12,
...
server <- function(input, output) {
  ...
  output$fig_map_dist <- renderPlot({
    ggplot() +
      ...
      geom_polygon(
        data = dataset_map_dist(),
        aes(x = long, y = lat, group = group),
        color = "black",
        # "mediumseagreen", #seagreen springgreen4
        fill = "gold1",
        lwd = 1.5
      ) +
      ggthemes::theme_few() +
  ...
  output$fig_dist <- renderPlot({
    ...
    dataset4 <- reactive({
      ...
      group_by(prob_nice) %>%
        ...
        distinct() %>%
        mutate(prob_nice = as.character(prob_nice)) %>%
        pivot_wider(names_from = prob_nice, values_from = ccbio_lbs)
